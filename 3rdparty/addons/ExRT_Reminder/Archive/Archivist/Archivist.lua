...
function Archivist:Initialize(sv)
	...
	for id, prototype in next, self.prototypes do
...
function Archivist:RegisterStoreType(prototype)
	...
	if self:IsInitialized() then
		...
		if oldPrototype then
			for storeID, store in next, self.activeStores[prototype.id] do
...
function Archivist:DeleteAll(storeType)
	if storeType then
		self.sv[storeType] = {}
		for id, store in next, self.activeStores[storeType] do
			self.activeStores[storeType][id] = nil
			self.storeMap[store] = nil
		end
	else
		for id in next, self.prototypes do
			self.sv[id] = {}
...
function Archivist:CloseAllStores()
	for storeType, prototype in next, self.prototypes do
		for id, store in next, self.activeStores[storeType] do
...
function Archivist:Commit(storeType, id)
	...
	local saved = self.sv[storeType][id]
	if image ~= nil then
		saved.data = self:Archive(image,true)
		saved.timestamp = time()
	end
end
